...
export default function AuthModal({ isOpen, onClose, defaultMode = 'signin' }: AuthModalProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Note: OAuth redirect will happen, so we don't close the modal here
    } catch (err) {
      setError(t('operationError'));
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="relative w-full max-w-md mx-4 glass rounded-2xl border border-slate-200/60 shadow-xl p-6">
        ...
        {/* Google Sign In Button - Only show for signin and signup modes */}
        {mode !== 'reset' && (
          <>
            ...
            <button
              ...
              className="w-full py-2.5 bg-white border border-slate-200 text-slate-700 rounded-lg font-medium hover:bg-slate-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 shadow-sm"
            >
              {googleLoading ? (
